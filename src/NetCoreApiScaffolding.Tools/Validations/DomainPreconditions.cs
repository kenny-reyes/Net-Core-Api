...
namespace NetCoreApiScaffolding.Tools.Validations
{
    public static class DomainPreconditions
    {
        ...
        public static void RegexMatch(string value, Regex regex, string parameterName, string entityName)
        {
            var match = regex.Match(value);
            if (!match.Success)
            {
                throw new DomainException(DomainPreconditionMessages.GetSuccessMatch(parameterName, entityName));
            }
        }
        ...
        public static void IsIntInIntList(IEnumerable<int> intList, int value, string parameterName)
        {
            ...
            {
                throw new DomainException(DomainPreconditionMessages.IsInTheList(parameterName));
            }
        }

        public static void IsStringInStringList(IEnumerable<string> stringList, string value, string parameterName)
        {
            if (stringList.All(i => i != value))
            {
                throw new DomainException(DomainPreconditionMessages.IsInTheList(parameterName));
            }
        }
